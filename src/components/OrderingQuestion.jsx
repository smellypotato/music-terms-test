...
function OrderingQuestion({ 
  question, 
  order, // Array of term IDs in current order
  onOrderChange, // Callback: (newOrder: string[]) => void
  questionIndex, 
  isDebugMode, 
  getTermGrade 
}) {
  ...
  // Determine ordering type from question text (display logic only)
      ...
      <div className="question-header">
        ...
        {isDebugMode && (
          <div className="debug-grade">
            Grade: {termIds.map(id => getTermGrade(id)).filter(g => g !== null).join(', ')}
          </div>
        )}
      </div>
